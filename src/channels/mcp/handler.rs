...
pub async fn mcp_post_handler(
    ...
    // workspace_read_scopes is set on the old base; skip for now.
...
mod tests {
    ...
    /// Verify the MCP handler enriches `JobContext` with timezone
    /// from the authenticated user and gateway state.
    #[test]
