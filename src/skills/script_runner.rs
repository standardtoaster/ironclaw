...
mod tests {
    ...
    #[tokio::test]
    async fn test_script_output_truncated() {
        let script = ActivationScript {
            language: "bash".to_string(),
            // Generate more than 32 bytes of output
            source: Some("printf '%0.s-' {1..100}".to_string()),
            source_file: None,
            timeout_ms: 5000,
            max_output_bytes: 32,
        };
    ...
    async fn test_python_script() {
        let script = ActivationScript {
            ...
            source: Some("print('from python')".to_string()),
            source_file: None,
            timeout_ms: 5000,
            max_output_bytes: 4096,
        };
        let result =
            run_activation_script(&script, Path::new("/tmp"), 3003, "test-token", "test-user")
                .await;
        assert_eq!(result, Some("from python".to_string()));
    }

    #[tokio::test]
    async fn test_source_file_not_found_returns_none() {
        let script = ActivationScript {
            language: "bash".to_string(),
            source: None,
            source_file: Some("nonexistent.sh".to_string()),
            timeout_ms: 5000,
            max_output_bytes: 4096,
        };
        let result =
