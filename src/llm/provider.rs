...
pub trait LlmProvider: Send + Sync {
    ...
    /// Shut down all sessions managed by this provider.
    ///
    /// Called during graceful application shutdown to clean up provider resources
    /// (e.g., supervisor-managed Claude sessions). Default is a no-op.
    async fn shutdown(&self) {
        // No-op for stateless providers.
    }
}
