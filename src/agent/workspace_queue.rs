...
impl WorkspaceQueueManager {
    ...
    pub async fn process_message(
        ...
        let result = async {
            ...
            metadata_map.insert("user_id".to_string(), serde_json::json!(user_id));
            metadata_map.insert(
                "conversation_id".to_string(),
                serde_json::json!(conversation_id.to_string()),
            );
            if let Some(serde_json::Value::Object(m)) = job_metadata {
                for (k, v) in m {
            ...
            // Job dispatched successfully — scheduler runs it asynchronously.
            // TODO: await_job support for synchronous workspace queue processing
            Ok::<String, String>(format!("Job {job_id} dispatched to workspace"))
        }
        .await;
