...
impl Tool for CollectionListTool {
    ...
    async fn execute(
        ...
        // Collect from own scope (cross-scope resolution requires multi-scope reads feature).
...
impl Tool for CollectionQueryTool {
    ...
    async fn execute(
        ...
        // cross-scope resolution (own scope first, then empty scopes list).
...
mod tests {
    ...
    //
    // NOTE: These tests require workspace_read_scopes on JobContext, which is
    // part of the multi-scope reads feature (separate stack band). The tests
    // are temporarily disabled until that feature is ported.
    //
    // TODO: Re-enable when multi-scope reads are ported to this base.

    // Disabled: requires workspace_read_scopes from multi-scope reads feature
    #[cfg(any())]
    mod cross_scope {
        use std::collections::BTreeMap;
        use std::sync::Arc;
        ...
        async fn resolve_scope_own_collection_first() {
            let tmp = TempDir::new().unwrap();
            let db = setup_db_with_collection("alice", "inventory", &tmp).await;
        ...
        async fn resolve_scope_falls_through_to_read_scope() {
            let tmp = TempDir::new().unwrap();
            let db = setup_db_with_collection("shared", "inventory", &tmp).await;
            ...
            assert_eq!(
                ...
                "should fall through to shared scope"
            );
        }
        ...
        async fn resolve_scope_returns_none_when_not_found() {
            let tmp = TempDir::new().unwrap();
            let db = setup_db_with_collection("carol", "inventory", &tmp).await;
        ...
        async fn query_tool_reads_from_cross_scope() {
            ...
            // alice has workspace_read_scopes = ["shared"] but no own inventory collection.
            let mut ctx = JobContext::with_user("alice", "test", "test");
            ctx.workspace_read_scopes = vec!["shared".to_string()];

            let result = tool.execute(serde_json::json!({}), &ctx).await.unwrap();
            let output = &result.result;
            assert_eq!(output["count"], 1, "should find shared scope's record via cross-scope");
            assert_eq!(output["results"][0]["data"]["item"], "widget");
        }
        ...
        async fn summary_tool_reads_from_cross_scope() {
            ...
            let result = tool
                ...
                .unwrap();
            let output = &result.result;
            // The aggregation result should reflect 2 records from shared scope.
            // Count returns the count directly (not wrapped in {"total": N}).
            assert_eq!(output["aggregation"], 2, "should count shared scope's records via cross-scope");
        }
        ...
        async fn list_tool_includes_cross_scope_collections() {
            ...
            assert!(names.contains(&"inventory"), "should include cross-scope collection");
        }
    }
    ...
    mod scoped_access {
        ...
        /// Set up a file-backed db without registering any collection.
        async fn test_db(tmp: &TempDir) -> Arc<dyn Database> {
            let db_path = tmp.path().join("test.db");
            let backend = LibSqlBackend::new_local(&db_path).await.unwrap();
            backend.run_migrations().await.unwrap();
            Arc::new(backend)
        ...
        async fn own_scope_add_writes_to_caller() {
            let tmp = TempDir::new().unwrap();
            let db = setup_db_with_collection("andrew", "personal_tasks", &tmp).await;
        ...
        async fn personal_item_does_not_leak_to_household() {
            ...
            let tool = CollectionAddTool::new(personal_schema, Arc::clone(&db), None);
        ...
        async fn household_item_does_not_leak_to_personal() {
            ...
            let tool = CollectionAddTool::new(household_schema, Arc::clone(&db), None);
        ...
        #[tokio::test]
        async fn own_scope_lens_gets_no_cross_scope_tools() {
            // When source_scope is None, the tool name should NOT have a
            // scope prefix — it's just "{collection}_{op}".
            ...
            let tmp = TempDir::new().unwrap();
            let db = setup_db_with_collection("andrew", "tasks", &tmp).await;

            let add_tool = CollectionAddTool::new(schema.clone(), Arc::clone(&db), None);
            assert_eq!(add_tool.name(), "tasks_add", "tool name should have no prefix");

            let query_tool = CollectionQueryTool::new(schema.clone(), Arc::clone(&db));
            assert_eq!(query_tool.name(), "tasks_query");

            let update_tool = CollectionUpdateTool::new(schema.clone(), Arc::clone(&db));
            assert_eq!(update_tool.name(), "tasks_update");

            let delete_tool = CollectionDeleteTool::new(schema, Arc::clone(&db));
            assert_eq!(delete_tool.name(), "tasks_delete");
        }
