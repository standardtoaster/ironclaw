...
async fn write_to_private_invisible_from_shared_scope() {
    ...
    // Write to private layer (scope = "alice")
    ws.write_to_layer("private", "notes/secret.md", "Private data", false)
        .await
        .expect("write should succeed");

    // A workspace scoped to "shared" must not see the private content
    let ws_shared = Workspace::new_with_db("shared", db_clone);
...
async fn write_to_shared_invisible_from_private_scope() {
    ...
    // Write to shared layer (scope = "shared")
    ws.write_to_layer("shared", "plans/visible.md", "Shared plan", false)
        .await
        .expect("write should succeed");

    // The primary scope (alice) should NOT see content written to the shared scope
    // unless multi-scope reads are configured (they aren't on this branch)
...
async fn write_empty_path_to_layer() {
    ...
    // Empty path should still work (normalize_path handles it)
    let result = ws
        .write_to_layer("private", "", "content", false)
        .await;
    // Either succeeds with a normalized path or fails gracefully — not a panic
...
async fn sensitive_write_to_private_layer_not_redirected() {
    ...
    // PII written directly to the private layer should NOT redirect (already private)
    let result = ws
